...
impl<'a> InferContext<'a> {
    ...
    fn finish(self) -> (Context, TypeResult<'a, ()>) {
        ...
        for (k, l) in env.iter() {
            if ctxt.contains_key(k) {
                return (ctxt, Err(TypeError::new(l.pos.clone(), DuplicateNameAt(k.as_ref().into(), None))));
            }
        }
    ...
    fn lookup_env(mut self, k: &'a Arc<str>, pos: &'a FilePos<'a>) -> Infer<'a, Type> {
        if let Some(s) = self.env.get(k).map(|s| &s.body).or_else(|| self.ctxt.get(k)).cloned() {
            ...
            Err((self.ctxt, TypeError::new(pos.clone(), UndefinedSymbol(k.as_ref().into()))))
        }
    }
    ...
    fn insert(mut self, Ident { body: k, pos }: &Ident<'a>, sc: Scheme, rewrite_ok: bool) -> Infer<'a, ()> {
        if let Some(v) = self.env.insert(k.clone(), Loc { pos: pos.clone(), body: sc }) {
            if !rewrite_ok {
                return Err((self.ctxt, TypeError::new(pos.clone(), DuplicateNameAt(k.as_ref().into(), Some(v.pos.clone())))));
            }
        }
        Ok((self, ()))
    }
